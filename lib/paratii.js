...
const Paratii = function (opts) {
  ...
  async function signMessage (message, prvKey) {
  
    // Elliptic curve signature must be done on the Keccak256 Sha3 hash of a piece of data.

    var data = ethUtil.toBuffer(message);
    var msgHash = ethUtil.hashPersonalMessage(data);  
    
    var privateKey = ethUtil.toBuffer(prvKey);

    var sig = ethUtil.ecsign(msgHash, privateKey);    
    var serialized = ethUtil.bufferToHex(ethSig.concatSig(sig.v, sig.r, sig.s))    

    return serialized

  }
  
  async function recoverAccount (sig, data) {
  ...
  async function isValidRequest (videoID, account, prvKey, signature) {

    var composedAttributes = videoID + '&account=' + account; 

    var requestData = ethUtil.toBuffer(composedAttributes);
    var requestHash = ethUtil.hashPersonalMessage(requestData);

    var privateKey = ethUtil.toBuffer(prvKey);
    
    var sig = ethUtil.ecsign(requestHash, privateKey);
