...
export class ParatiiEthUsers {
  ...
  async create (options) {
    ...
    await this.fixMethodAndSend(contract.methods.registerUser(options.id, options.name, options.email, options.ipfsHash))
    return options.id
  }
  ...
  async get (userId) {
    let contract = await this.getRegistry()
    contract.setProvider(this.eth.config.provider)
    let userInfo = await this.fixMethodAndCall(contract.methods.getUserInfo(userId))
    let result = {
      id: userId,
      name: userInfo[0],
      email: userInfo[1],
  ...
  async delete (userId) {
    let contract = await this.getRegistry()
    let tx = this.fixMethodAndSend(contract.methods.unregisterUser(userId))
    return tx
  }
}
