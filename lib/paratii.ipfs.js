  ...
  putJSON (data) {
    return new Promise((resolve, reject) => {
      if (!this.ipfs || !this.ipfs.isOnline()) {
        return reject(new Error('IPFS node is not ready, please trigger getIPFSInstance first'))
      }
      const obj = {
      ...
      this.ipfs.object.put(obj, (err, node) => {
        ...
        return resolve(node.toJSON().multihash)
        // callback(null, node)
      })
    })
  }
  ...
  getJSON (multihash) {
    return new Promise((resolve, reject) => {
      ...
      this.ipfs.object.get(multihash, (err, node) => {
        ...
        // console.log(node.toJSON().multihash)
        return resolve(JSON.parse(node.toJSON().data))
        // callback(null, node)
      })
    })
  }
