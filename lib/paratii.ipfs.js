...
export class ParatiiIPFS {
  ...
  putJSON (data, callback) {
    if (!this.ipfs || !this.ipfs.isOnline()) {
      return callback(new Error('IPFS node is not ready, please trigger getIPFSInstance first'))
    }
    const obj = {
    ...
    this.ipfs.object.put(obj, (err, node) => {
      if (err) {
        return callback(err)
      }
      ...
      callback(null, node.toJSON().multihash)
      // callback(null, node)
    })
  }

  getJSON (multihash, callback) {
    if (!this.ipfs || !this.ipfs.isOnline()) {
      return callback(new Error('IPFS node is not ready, please trigger getIPFSInstance first'))
    }

    this.ipfs.object.get(multihash, (err, node) => {
      if (err) {
        return callback(err)
      }

      console.log('got node: ', node.toJSON())
      console.log(node.toJSON().multihash)
      callback(null, JSON.parse(node.toJSON().data))
      // callback(null, node)
    })
  }
