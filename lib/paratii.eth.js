...
export class ParatiiEth {
  constructor (config) {
    ...
    if (options.web3) {
      ...
      this.web3 = new Web3()
      if (config.wsprovider === undefined) {
        this.web3.setProvider(new this.web3.providers.HttpProvider(options.provider))
      } else {
        this.web3.setProvider(new Web3.providers.WebsocketProvider(config.wsprovider))
      }
    }
  ...
  async deployContracts () {
    ...
    await paratiiRegistry.methods.registerAddress('Avatar', paratiiAvatar.options.address)
    await paratiiRegistry.methods.registerAddress('ParatiiToken', paratiiToken.options.address)
    await paratiiRegistry.methods.registerAddress('SendEther', sendEther.options.address)
    await paratiiRegistry.methods.registerAddress('Videos', videoRegistry.options.address)
    await paratiiRegistry.methods.registerAddress('Store', videoStore.options.address)
    await paratiiRegistry.methods.registerAddress('Users', userRegistry.options.address)
