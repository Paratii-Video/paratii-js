...
export class ParatiiEthVids {
  ...
  async create (options) {
    ...
    let tx = await this.fixMethodAndSend(contract.methods.create(options.id, options.owner, options.price, options.ipfsHash, options.ipfsData))
  ...
  async delete (videoId) {
    let contract = await this.getRegistry()
    contract.setProvider(this.eth.config.provider)
    let tx = await this.fixMethodAndSend(contract.methods.remove(videoId))
    return tx
  }
}
