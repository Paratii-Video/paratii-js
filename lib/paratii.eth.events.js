  ...
  async _addListener (eventType, listener, options) {
    ...
    switch (structuredEvent.event) {
      ...
      case 'pendingTransactions': {
        subscription = this.subscribe(eventType, function (error, event) {
          if (error) {
            throw error
          }
          event.on(structuredEvent.emit, listener)
        })
        break
      }
      case 'logs': {
        ...
        subscription = this.subscribe('logs', options, function (error, event) {
          if (error) {
            throw error
          }
          event.on(structuredEvent.emit, listener)
      ...
      default: {
        console.log(this._customEvents[structuredEvent.event].contract)
        console.log(this._customEvents[structuredEvent.event].event)
