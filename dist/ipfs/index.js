...
var paratiiIPFS = {
  ...
  */
  ...
  triggerOnReady: function triggerOnReady() {
    ...
    paratiiIPFS.onReadyHook.forEach(function (func) {
      // VALIDATION IS REQUIRED HERE>>>>>>>>>>>>
      console.info('Triggering ', func);
      func();
    });
  },
  addOnReady: function addOnReady(func) {
  ...
  setRepoPath: function setRepoPath(path) {
    if (window.localStorage) {
  ...
  clearRepo: function clearRepo(callback) {
  ...
  // transactions
  updateTransactions: function updateTransactions(callback) {
    ...
    var localLedger = window.localStorage.getItem('paratii-ledger');
    if (!localLedger) {
      localLedger = {};
    } else {
   ...
   */
  getTransactions: function getTransactions(peerId, callback) {
   ...
   */
  clearTransactionsCache: function clearTransactionsCache(callback) {
    window.localStorage.removeItem('paratii-ledger', null);
    (0, _lodash.setImmediate)(callback);
  },
   ...
   */
  meterController: function meterController(peer, size, callback) {
    if (!callback) {
    ...
    var CREDIT_PER_PEER = 5529887 / 2;
    paratiiIPFS.getTransactions(peer, function (err, ledger) {
      if (err) throw err;
