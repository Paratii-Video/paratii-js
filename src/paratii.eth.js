 ...
 */
  ...
  requireContract (contractName) {
    let contract, pathToContractInfo
    let from = this.config.account.address

    let contractArr = contractName.split('/')
    if (contractArr[0] === 'sol-tcr') {
      pathToContractInfo = path.join(require.resolve('sol-tcr'), '..', `/build/contracts/${contractArr[1]}.json`)
    } else {
      pathToContractInfo = path.join(require.resolve('paratii-contracts'), '..', `/build/contracts/${contractName}.json`)
    }

    const content = fs.readFileSync(pathToContractInfo, 'utf8')
  ...
  async deployContracts () {
    const content = fs.readFileSync(this.config.eth.tcrConfigFile, 'utf8')
    const tcrConfig = JSON.parse(content)
    ...
    let tcrRegistry = await this.deployWithLinks(
      'TcrRegistry',
      {
        DLL: tcrDLL.options.address,
        AttributeStore: tcrAttributeStore.options.address
