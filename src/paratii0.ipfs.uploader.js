...
export class ParatiiIPFSUploader extends EventEmitter {
  ...
  */
  ...
  pinFile (fileHash, options) {
    ...
    const schema = joi.object({
      author: joi.string().default('0x'), // ETH/PTI address of the file owner
      remoteIPFSNode: joi.string().default(this.config.ipfs.remoteIFPFSNode),
      remoteIPFSNodeId: joi.any().default(Multiaddr(this.config.ipfs.remoteIFPFSNode).getPeerId()),
